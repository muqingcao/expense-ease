...
//  ExpenseEase
//
//  Created by Xihuan Liu on 11/20/24.
//
...
extension EditExpenseViewController {
    func setupCategoryButtons() {
        ...
        editExpenseView.categoriesStackView.addArrangedSubview(gridStackView)
    }
    ...
    func createCategoryGrid(titles: [String], imageNames: [String]) -> UIStackView {
        ...
        highlightSelectedButton(selectedCategory: selectedCategory)

        return stackView
    }
    
    @objc func highlightSelectedButton(selectedCategory: String) {
        for (index, button) in categoryButtons.enumerated() {
            if Categories.categoryTitles[index] == selectedCategory {
